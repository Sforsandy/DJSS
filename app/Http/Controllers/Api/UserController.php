...
class UserController extends Controller
{
    ...
    public function login(){
        ...
        if(Auth::attempt(['mobile_no' => request('mobile_no'), 'password' => request('password')])){
            ...
            {
                $refer_code = strtoupper(substr($user->firstname, 0, 3).bin2hex(random_bytes(1)).rand(100,999));
                $user = User::find($user->id);
    ...
    public function ValidateUser(Request $request)
    {
        ...
        if($chkEmail > 0)
        {
            return response()->json(['success'=>'0','message'=>'The email has already beed taken.'], $this->successStatus);
        }
        $otp = rand(1000, 9999);
        $token = Str::random(25);
        $SmsVerification = new SmsVerification();
